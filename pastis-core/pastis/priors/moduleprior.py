...
def compute_prior(
    ...
    **kwargs
    ...
    results = np.genfromtxt(model_file, skip_header=83, skip_footer=4)
    ...
    ### BEB
    #######
    pBEB, pBB, nBin, nEB, nStars, nRegions = proba_BEB(
        results,
        mag_target + deltamag,
        mag_max,
        mag_target,
    ...
    ### BTP
    #######
    pBTP, pBP, nPla, nTP, nStars, nRegions = proba_BTP(
        results,
        mag_target + deltamag,
        mag_max,
        mag_target,
    ...
    print("p(BB): %e" % pBB)
    print("p(BEB): %e +/- %e" % (np.mean(pBEB), np.std(pBEB)))
    ...
    print("p(BP): %e" % pBP)
    print("p(BTP): %e +/- %e" % (np.mean(pBTP), np.std(pBTP)))

    print("=" * 80)
    print("PiB : transiting planetary system in binary system")
    print("p(PiB): %e +/- %e" % (np.mean(pPIB), np.std(pPIB)))
    ...
    print(
        "Number of eclipsing binary stars in simulation: %e +/- %e"
        % (np.mean(nEB), np.std(nEB))
    )
    print("Number of planetary systems in simulation: %e" % nPla)
    print("Number of transiting planetary systems in simulation: %e" % np.mean(nTP))
...
def proba_BEB(
    ...
    """
    magbins = np.arange(mag_min, mag_max + binsize, binsize)
    ...
    R1 = np.sqrt(L * (5777.0 / Teff) ** 4.0)  # In solar units
    ...
    nBin = np.zeros(len(magbins) - 1)
    pBB = np.zeros(len(magbins) - 1)

    nEB = np.zeros((niterations, len(magbins) - 1))
    pBEB = np.zeros((niterations, len(magbins) - 1))

    nstars = np.zeros(len(magbins) - 1)
    nRegions = np.zeros(len(magbins) - 1)
    ...
    for j, mag in enumerate(magbins[:-1]):
        ...
        for i in range(niterations):
            ...
            nEB[i, j] = np.sum(bin_prob[cond] * eclip_prob[cond])
    ...
    return map(np.sum, (pBEB, pBB, nBin, nEB, nstars, nRegions), (1, 0, 0, 1, 0, 0))
    """
...
def proba_BTP(
    ...
    #### Prepare arrays containing final result
    nPlanet = np.zeros(len(magbins) - 1)
    pBP = np.zeros(len(magbins) - 1)

    nTP = np.zeros(len(magbins) - 1)
    pBTP = np.zeros(len(magbins) - 1)
    ...
    for j, mag in enumerate(magbins[:-1]):
        ...
        # of individual stars
        nPlanet[j] = np.sum(planet_prob[cond])
        ...
        nTP[j] = np.sum(planet_prob[cond] * transit_prob[cond])
    ...
    ## Return sum over bins
    return map(np.sum, (pBTP, pBP, nPlanet, nTP, nstars, nRegions))
...
def proba_PIB(sp_type, M1, period, error_period, Rp=2, niterations=100):
    ...
    # Prepare arrays
    pPla = np.zeros(niterations)
    pTP = np.zeros(niterations)
    M2 = np.zeros(niterations)
...
def proba_PLA(mass1, radius1, radius2, period, error_period):
    ...
    if radius2 >= 6 and radius2 < 22:
        ...
        raise ValueError(
            "Radius of transiting planet too large; has to be smaller than 22 Rearth"
        )
...
def proba_planet_spt(sp_type, period, error_period=0.0):
    ...
    planet_prob = np.zeros(len(sp_type))
...
def semi_major_axis(mass1, mass2, period):
    a = (
        (G * ((mass1 + mass2) * Msun) * (period * 24 * 3600) ** 2) / (4.0 * np.pi**2)
    ) ** (1.0 / 3.0)
    return a / Rsun
...
def massradiusrel(M):
    ...
    """
    if (np.log10(M) > 1.3).any() or (np.log10(M) < -1.0).any():
    ...
    radius = np.where(
        np.log10(M) > 0.12,
        10 ** (0.640 * np.log10(M) + 0.011),
        10 ** (0.917 * np.log10(M) - 0.020),
    )
