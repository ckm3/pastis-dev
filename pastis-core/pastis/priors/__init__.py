...
def run_prior(pastisfile):
    ...
    ec = ephem.Equatorial(
        infodict["alpha"] * pi / 180.0, infodict["delta"] * pi / 180.0
    )
    ...
    if not os.path.exists(file_name):
        dm.besancon_model_form(
            name_target=infodict["name"],
...
def initialize_priors(pastisfile):
    ...
    if infodict["PS"] == "Mayor":
        ...
        planet_period_dist = {}
        for ptype in ["Earth", "SuperEarth", "SmallNeptune", "LargeNeptune", "Jupiter"]:
            if ptype != "Jupiter":
    ...
    ### Fressin statistics ####
        ...
        G, LN, SN, SE, E = np.loadtxt(
            PRIORdict[infodict["PS"]][0],
            skiprows=2,
            unpack=True,
            usecols=(0, 2, 4, 6, 8),
        )
        ...
        peredges = np.loadtxt(PRIORdict[infodict["PS"]][1], skiprows=2)
        ...
        for ptype, pdist in zip(
            ("Earth", "SuperEarth", "SmallNeptune", "LargeNeptune", "Jupiter"),
            (E, SE, SN, LN, G),
        ):
            ##
            cond = np.array(map(np.isfinite, pdist))
            planet_period_dist[ptype] = interpolate.interp1d(
                P[cond], pdist[cond] * 1e-2
            )
    ...
    rdist, deltamag = np.loadtxt(
        datadict["HYPpriors"]["sensifile"], unpack=True, skiprows=2
    )
    contrast_curve = interpolate.interp1d(
        deltamag, rdist, fill_value=np.max(rdist), bounds_error=False
    )
    ...
    EXTfile = os.path.join(
        extpath,
        "ext_ra%.3f_dec%.3f_max%.d_step%.1f.txt"
        % (
            infodict["alpha"],
            infodict["delta"],
            infodict["MaxDist"],
    ...
    ext_curve = interpolate.interp1d(
        dist, redening, bounds_error=False, fill_value=np.max(redening)
    )
