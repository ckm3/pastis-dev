...
def interpol_tracks(input_file):
    ...
    z, mini, mact, logage, logT, logg, logL = loadtxt_iter(
...
def prepare_tracks_target(input_file, AgeUniverse=10.4):
    ...
    z, mini, mact, logage, logT, logg, logL, phase = loadtxt_iter(
        input_file, usecols=(2, 3, 4, 5, 6, 7, 8, 9), unpack=True, skiprows=1
    )
...
def get_stellarparams_target(z, y, logT, N=4, Nt=10, planethost=False):
    ...
    for ip in range(vals0.shape[1]):
        valsint[ip] = interpolate.griddata(
...
def interpol_WD(input_file):
    ...
    teff, logg, mact, age = loadtxt_iter(
