def fithist(y, par0, nbins=50, disttype="Normal", plot=True, **kwargs):
    ...
    # Construct histogram
    ni, binedges = np.histogram(y, bins=nbins)
    x = binedges[:-1] + 0.5 * np.diff(binedges)

    # Estimated number of events
    distdict[disttype][0].pdf(x, *par0) * np.diff(binedges)[0] * len(y)
    ...
    def residuals(par, x, ni, bin_edges):
        ...
        # Estimated number of events
        ni0 = distdict[disttype][0].pdf(x, *par) * np.diff(bin_edges)[0] * ntotal

        return (ni - ni0) / np.sqrt(ni0)

    popt, epopt, infodict, mesg, wf = optimize.leastsq(
        residuals, par0, args=(x, ni, binedges), full_output=True, **kwargs
    )
    ...
    if plot:
        ...
        ni0 = distdict[disttype][0].pdf(x, *popt) * np.diff(binedges)[0] * ntotal

        ax.hist(y, nbins, histtype="step", lw=2, color="k", normed=False)
        ax.errorbar(x, ni, np.sqrt(ni0), fmt="ko")

        xx = np.linspace(x.min(), x.max(), 1000)
        ax.plot(
            xx,
            distdict[disttype][0].pdf(xx, *popt) * np.diff(binedges)[0] * ntotal,
            lw=2,
            color="r",
        )
