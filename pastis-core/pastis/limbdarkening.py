...
def initialize_limbdarkening(pbands, ATMmodel="A", LDCfile=LDdict["Claret2011_wTESS"]):
...
def interpol_LD(LDCfile, photbands=["Kepler", "CoRoT"], ATMmodel="A"):
    ...
    for i, line in enumerate(lines[32:-2]):
        try:
            ll = line.split()
            logg[i], Teff[i], z[i], microturb[i], a[i], b[i] = list(map(float, ll[:6]))
...
def get_LD(teff, logg, z, photband, verbose=False):
    ...
    if teff > 50000.0:
        teff = 50000.0
        print_warning = True
        wstring = ["Teff", "maximum", 50000]

    if teff < 3500.00:
        teff = 3500.00
        print_warning = True
        wstring = ["Teff", "minimum", 3500]

    if logg > 5.0:
        logg = 5.0
        print_warning = True
        wstring = ["logg", "maximum", 5.0]

    if logg < 0.0:
        logg = 0.0
        print_warning = True
        wstring = ["logg", "minimum", 0.0]

    if z > 1.0:
        z = 1.0
        print_warning = True
        wstring = ["z", "maximum", 1.0]

    if z < -2.5:
        z = -2.5
        print_warning = True
        wstring = ["z", "minimum", -2.5]
    ...
    if photband == "CoRoT-R" or photband == "CoRoT-G" or photband == "CoRoT-B":
        ...
        return [
            phot.CoRoT_LDC_weights[ccc, 0] * ldcu[0]
            + phot.CoRoT_LDC_weights[ccc, 1] * ldcg[0]
            + phot.CoRoT_LDC_weights[ccc, 2] * ldcr[0]
            + phot.CoRoT_LDC_weights[ccc, 3] * ldci[0]
            + phot.CoRoT_LDC_weights[ccc, 4] * ldcz[0],
            phot.CoRoT_LDC_weights[ccc, 0] * ldcu[1]
            + phot.CoRoT_LDC_weights[ccc, 1] * ldcg[1]
            + phot.CoRoT_LDC_weights[ccc, 2] * ldcr[1]
            + phot.CoRoT_LDC_weights[ccc, 3] * ldci[1]
            + phot.CoRoT_LDC_weights[ccc, 4] * ldcz[1],
        ]
    else:
