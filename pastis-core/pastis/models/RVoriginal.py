...
#lib = cdll.LoadLibrary(os.path.dirname(os.path.realpath(__file__)) + "/fitgauss_c.so")
...
def Vasy(RV, CCF, Gauss_params):
	...
	depth, lambda_b, lambda_r, v0 = CCF_interpolate(RV-center_G, CCF, Gauss_params)
...
def asym_gaussian(p,x):
	...
	p : [c, rv0, sig, Asym]
	"""
	Gb = 1. - p[0]/100.*np.exp(-(x[np.where(x < p[1])[0]]-p[1])**2/(2.*(p[2]*(1.-p[3]))**2))
	Gr = 1. - p[0]/100.*np.exp(-(x[np.where(x >= p[1])[0]]-p[1])**2/(2.*(p[2]*(1.+p[3]))**2))
	return np.concatenate((Gb,Gr))

def gaussian(p ,x): 
	"""
	p : [c, rv0, sig]
	"""
	return 1. - p[0]/100.*np.exp(-(x-p[1])**2/(2.*p[2]**2))
...
def gaussian_res(p ,x, y): 
	rez = np.empty(x.size)
	lib.gaussian_res(p.ctypes.data_as(POINTER(c_double)), x.ctypes.data_as(POINTER(c_double)), 
	y.ctypes.data_as(POINTER(c_double)), len(x), rez.ctypes.data_as(POINTER(c_double)));
	return rez;

def gaussian_res_J(p ,x, y):
	rez = np.empty((4, x.size))
	lib.gaussian_res_J(p.ctypes.data_as(POINTER(c_double)), x.ctypes.data_as(POINTER(c_double)), 
	y.ctypes.data_as(POINTER(c_double)), len(x), rez.ctypes.data_as(POINTER(c_double)));
	return rez
...
def fitgauss_old(x,y,p, mode='normal'):
	...
	from scipy import optimize
	if mode == 'normal' : 
	    p1, wf = optimize.leastsq(res, p, args=(x,y),full_output=False,xtol=1e-5) # precision required of 10cm/s
	elif mode == 'asym' : 
		p1, wf = optimize.leastsq(asym_res, p, args=(x,y),full_output=False,xtol=1e-5) # precision required of 10cm/s
	if not wf in (1,2,3,4):
...
def fitgauss(x,y,p, mode='normal'):
	...
	if mode == 'normal' : 
		...
		p1,wf = optimize.leastsq(gaussian_res, p, Dfun = gaussian_res_J, args=(x,y), col_deriv = True, full_output = False, xtol=xtol);
		if not wf in (1,2,3,4):
			print 'Warning: problem with RV gaussian fit'
			return [999.,999.,999.], 0
		else:
...
def vspan(x, y, p, mode = 'TB'):
	...
	if ll1>x[0] and ll4<x[-1]:
		...
		if mode == 'TB' :
			x1=np.concatenate((x[:ind2],x[ind3+1:]))
			y1=np.concatenate((y[:ind2],y[ind3+1:]))
			x2=np.concatenate((x[:ind1],x[ind2:ind3+1],x[ind4+1:]))
			y2=np.concatenate((y[:ind1],y[ind2:ind3+1],y[ind4+1:]))
		elif mode == 'RB' :
			x1=np.concatenate((x[:ind5],x[ind4:]))
			y1=np.concatenate((y[:ind5],y[ind4:]))
			x2=np.concatenate((x[:ind1],x[ind5:]))
			y2=np.concatenate((y[:ind1],y[ind5:]))
		...
		try : 
			...
			if conv_5 and conv_6 : vspan=CCF_RV_high-CCF_RV_low
			else : vspan=999.
		except : vspan = 999.
	else : vspan = 999.
	return vspan
...
def fit_BiGauss(x,y,p):
	...
	p : [c, rv0, sig]
	"""
	return p[1] - fitgauss(x,y,np.hstack([p,[0.0]]), mode='asym')[0][1]
 
def fit_BIS(x, y, p):
	"""
	Return the Bisector Inverse Slope (BIS) as defined in Queloz et al. (2001)
	"""
	seuil=np.linspace(0., 1., 101)
	y -= min(y)
	y/=max(y)
	left = np.where(np.logical_and(x <= p[1], x > p[1] - 5.*p[2]))[0]
	right = np.where(np.logical_and(x >= p[1], x < p[1] + 5.*p[2]))[0]
	Vleft = np.interp(seuil, 1.-y[left], x[left])
	Vright = np.interp(seuil, y[right], x[right])
	Vtop = 0.
...
def get_FWHM(vsini, BmV, spectrograph, output = 'FWHM'):
	...
	if spectrograph == 'CORALIE' : 
		...
		A=1.9
		FWHM=np.sqrt((vsini**2.+A**2.*sigma0**2.)/A**2.)*(2.*np.sqrt(2.*np.log(2.)))
	if spectrograph == 'HARPS' : 
		sigma0=8.625-20.037*BmV+23.388*BmV**2-10.364*BmV**3+1.273*BmV**4
		A=1.95
		FWHM=np.sqrt((vsini**2.+A**2.*sigma0**2.)/A**2.)*(2.*np.sqrt(2.*np.log(2.)))
	...
	if output == 'FWHM' : return FWHM
	elif output == 'sigma0' : return sigma0
	elif output == 'sigma' : return FWHM/(2.*np.sqrt(2.*np.log(2.)))
	else : return
...
def get_contrast(FWHM, BmV, z, spectrograph, mask, output = 'ctrs'):
	...
	if spectrograph == 'HARPS':
		...
		logW=c[0]+c[1]*BmV+c[2]*z+c[3]*BmV**2+c[4]*z**2+c[5]*BmV*z
	...
	if spectrograph.find('CORALIE')>-1:
		...
		if mask == 'K0':
			logW = (z - 2.573 + 8.142*BmV - 5.583*BmV**2) / 4.587 #Santos et al. 2002

	if output == 'ctrs' : return 10**logW / np.sqrt(2.*np.pi) / (FWHM / (2.*np.log(2.))) * 100. # in % 
	elif output == 'W' : return 10**logW
	else : return
...
def get_Wrot(ctrs, Wccf, rv, vsini, epsilon, FWHM_instru):
	...
	return Wccf - integrate.trapz(1. - profile_rot(rv, 0., vsini, epsilon, ctrs[0], FWHM_instru), rv)
...
def get_contrast_rot(rv, vsini, BmV, z, spectrograph, mask, epsilon):
	...
	CRot, toto = optimize.leastsq(get_Wrot, [1.], args=(Wccf, rv, vsini, epsilon, FWHM_instru))
	return CRot
...
def profile_rot(x, x0, vsini, epsilon, ctrs, FWHM_instru = 9.9):
	...
	y1=1./np.sqrt(2.*np.pi*(FWHM_instru/sig)**2)*np.exp(-(x-x0)**2/(2.*(FWHM_instru/sig)**2))
	#y1=n.exp(-(x-x0)**2/(2.*(FWHM_instru/sig)**2))*ctrs
	y2=rot_profile(x,vsini,epsilon)
	#y2/=max(y2)
	Conv2=np.convolve(y1[1:],y2[:-1],mode=1)
	Conv2 = np.array(list(Conv2) + [0.0])
	Conv2 /= max(Conv2)
	CCF = (1. - Conv2*ctrs)
	#CCF -= min(CCF) - (1. - ctrs)
	#CCF /= max(CCF)
...
def rot_profile(v, vsini, epsilon):
	...
	c1= 2. *(1.- epsilon)
	c2= 0.5 * np.pi * epsilon

	G=np.zeros(len(v), float)
...
def CCF_prop(t_rv, spectro, mask, *args):
 # count number of stars
	nb_star = 0
	for obj in args :
		if isinstance(obj, Star): nb_star+=1
		if isinstance(obj, IsoBinary): nb_star+=2
		if isinstance(obj, Triple):
			for comp in (obj.object1, obj.object2):
				if isinstance(comp, Star):
					nb_star+=1
				elif isinstance(comp, IsoBinary):
					nb_star+=2
				elif isinstance(comp, PlanSys):
					nb_star+=1
		if isinstance(obj, PlanSys): nb_star+=1
	FWHM = np.zeros(nb_star, float)
	contrast = np.zeros(nb_star, float)
	rv0 = np.zeros([nb_star, len(t_rv)], float)
	flux = np.zeros(nb_star, float)
	...
	for obj in args :
		if isinstance(obj, Star):
			...
			rv0[count] = obj.v0*np.ones(len(t_rv), float)
			count+=1
		...
		if isinstance(obj, Triple):
			for component, labelcomp in zip((obj.object1, obj.object2), ('object1', 'object2')):
				if isinstance(component, Star):
					component.BmV = component.get_BmV()
					FWHM[count] = get_FWHM(component.vsini, component.BmV, spectro)
					contrast[count] = get_contrast(FWHM[count], component.BmV, component.z, spectro, mask)
					...
					FWHM[count] = get_FWHM(component.star1.vsini, component.star1.BmV, spectro)
					contrast[count] = get_contrast(FWHM[count], component.star1.BmV, component.star1.z, spectro, mask)
					flux[count] = component.star1.get_flux('Johnson-V')
					rv0[count] = obj.get_RV(t_rv, component = labelcomp) + component.get_RV(t_rv, component = 'primary') - component.v0
					count+=1
					component.star2.BmV = component.star2.get_BmV()
					FWHM[count] = get_FWHM(component.star2.vsini, component.star2.BmV, spectro)
					contrast[count] = get_contrast(FWHM[count], component.star2.BmV, component.star2.z, spectro, mask)
					flux[count] = component.star2.get_flux('Johnson-V')
					rv0[count] = obj.get_RV(t_rv, component = labelcomp) + component.get_RV(t_rv, component = 'secondary') - component.v0
					count+=1
				elif isinstance(component, PlanSys):
					component.star.BmV = component.star.get_BmV()
					FWHM[count] = get_FWHM(component.star.vsini, component.star.BmV, spectro)
					contrast[count] = get_contrast(FWHM[count], component.star.BmV, component.star.z, spectro, mask)
					flux[count] = component.star.get_flux('Johnson-V')
					rv0[count] = obj.get_RV(t_rv, component = labelcomp) + component.get_RV(t_rv) - component.v0
					count+=1
...
def PASTIS_RV(t_rv, RVdatadict, *args):
    ...
    if RVdatadict.has_key('RV')\
        ...
        rv_simu = np.zeros(len(t_rv), float)
        fwhm_simu = np.zeros(len(t_rv), float)
        contrast_simu = np.zeros(len(t_rv), float)

    if RVdatadict.has_key('BIS'): bis_simu = np.zeros(len(t_rv), float)
    if RVdatadict.has_key('Vspan'): vspan_simu = np.zeros(len(t_rv), float)
    if RVdatadict.has_key('Wspan'): wspan_simu = np.zeros(len(t_rv), float)
    if RVdatadict.has_key('BiGauss'): bigauss_simu = np.zeros(len(t_rv), float)
    if RVdatadict.has_key('Vasy'): vasy_simu = np.zeros(len(t_rv), float)

    condFitBin = map(isinstance, args, [FitBinary]*len(args))
    condIsoBin = map(isinstance, args, [IsoBinary]*len(args))
    condFitPlanet = map(isinstance, args, [FitPlanet]*len(args))
    cond_drift = map(isinstance, args, [Drift]*len(args))

    if len(args) == 1 and isinstance(args[0], PlanSys):
        ...
        CCF_width = FWHM/(2.*np.sqrt(2.*np.log(2.)))
        if RVdatadict.has_key('RV')  :
            output_dict['RV'] = args[0].get_RV(t_rv) - RVdatadict['RV']['offset']
            for p in args[0].planets:
                if p.orbital_parameters.spinorbit <> None :
                    if isinstance(p, Planet) :output_dict['RV'] += Pyarome.arome(p.get_true_lat(t_rv), p.ar*(1.-p.orbital_parameters.ecc**2)/(1.+p.orbital_parameters.ecc*np.sin(p.orbital_parameters.omega)), p.orbital_parameters.incl, p.orbital_parameters.spinorbit, [args[0].star.ua, args[0].star.ub], beta0=sigma0, Vsini=args[0].star.vsini, sigma0=CCF_width, zeta=args[0].star.zeta, Rp=p.Rp/p.star.R, units='degree')
                    elif isinstance(p, FitPlanet) :output_dict['RV'] += Pyarome.arome(p.get_true_lat(t_rv), p.ar*(1.-p.orbital_parameters.ecc**2)/(1.+p.orbital_parameters.ecc*np.sin(p.orbital_parameters.omega)), p.orbital_parameters.incl, p.orbital_parameters.spinorbit, [args[0].star.ua, args[0].star.ub], beta0=sigma0, Vsini=args[0].star.vsini, sigma0=CCF_width, zeta=args[0].star.zeta, Rp=p.kr, units='radian')
        if RVdatadict.has_key('CTRS') : output_dict['CTRS'] = np.ones(len(t_rv), float) * contrast - RVdatadict['CTRS']['offset']
        if RVdatadict.has_key('FWHM') : output_dict['FWHM'] = np.ones(len(t_rv), float) * FWHM - RVdatadict['FWHM']['offset']
        if RVdatadict.has_key('BIS') : output_dict['BIS'] = np.zeros(len(t_rv), float) - RVdatadict['BIS']['offset']
        if RVdatadict.has_key('Vspan') : output_dict['Vspan'] = np.zeros(len(t_rv), float) - RVdatadict['Vspan']['offset']
        if RVdatadict.has_key('Wspan') : output_dict['Wspan'] = np.zeros(len(t_rv), float) - RVdatadict['Wspan']['offset']
        if RVdatadict.has_key('BiGauss') : output_dict['BiGauss'] = np.zeros(len(t_rv), float) - RVdatadict['BiGauss']['offset']
        if RVdatadict.has_key('Vasy') : output_dict['Vasy'] = np.zeros(len(t_rv), float) - RVdatadict['Vasy']['offset']
        ...
        if RVdatadict.has_key('CTRS') : output_dict['CTRS'] = np.zeros(len(t_rv), float) - RVdatadict['CTRS']['offset']
        if RVdatadict.has_key('FWHM') : output_dict['FWHM'] = np.zeros(len(t_rv), float) - RVdatadict['FWHM']['offset']
        if RVdatadict.has_key('BIS') : output_dict['BIS'] = np.zeros(len(t_rv), float) - RVdatadict['BIS']['offset']
        if RVdatadict.has_key('Vspan') : output_dict['Vspan'] = np.zeros(len(t_rv), float) - RVdatadict['Vspan']['offset']
        if RVdatadict.has_key('Wspan') : output_dict['Wspan'] = np.zeros(len(t_rv), float) - RVdatadict['Wspan']['offset']
        if RVdatadict.has_key('BiGauss') : output_dict['BiGauss'] = np.zeros(len(t_rv), float) - RVdatadict['BiGauss']['offset']
    ...
    elif all(np.array(condFitPlanet) | np.array(cond_drift)):
        for obj in args:
            rv_simu += obj.get_RV(t_rv)
            if isinstance(obj, FitPlanet) and obj.orbital_parameters.spinorbit is not None:
                sigma0 = get_FWHM(obj.vsini1, obj.BmV, spectro, output='sigma0')
                CCF_width = get_FWHM(obj.vsini1, obj.BmV, spectro, output='sigma')
                rv_simu += Pyarome.arome(obj.get_true_lat(t_rv), obj.ar*(1.-obj.orbital_parameters.ecc**2)/(1.+obj.orbital_parameters.ecc*np.sin(obj.orbital_parameters.omega)), obj.orbital_parameters.incl, obj.orbital_parameters.spinorbit, [obj.ua1, obj.ub1], beta0=sigma0, Vsini=obj.vsini1, sigma0=CCF_width, zeta=obj.zeta1, Rp=obj.kr, units='radians')
        if RVdatadict.has_key('RV')  : output_dict['RV'] = rv_simu - RVdatadict['RV']['offset']
        if RVdatadict.has_key('CTRS') : output_dict['CTRS'] = np.ones(len(t_rv), float) * contrast - RVdatadict['CTRS']['offset']
        if RVdatadict.has_key('FWHM') : output_dict['FWHM'] = np.ones(len(t_rv), float) * FWHM - RVdatadict['FWHM']['offset']
        if RVdatadict.has_key('BIS') : output_dict['BIS'] = np.zeros(len(t_rv), float) - RVdatadict['BIS']['offset']
        ...
        # CCF parameters
        cond_RV_CCF = np.logical_and(RV_CCF_ALL > np.min(rv0) - 5.*max(FWHM)/2.3548, RV_CCF_ALL < np.max(rv0) + 5.*max(FWHM)/2.3548)
        RV_CCF = RV_CCF_ALL[np.where(cond_RV_CCF)[0]]
        CCF = np.zeros([nb_star, len(RV_CCF)], float)

        # Make fixed CCFs
        count = 0
        CCF_sum = np.zeros(len(RV_CCF), float)
        while isinstance(args[count], Star):
            CCF[count] = make_CCF(RV_CCF, rv0[count,0], FWHM[count], contrast[count], flux[count])
            CCF_sum+=CCF[count]
            count+=1
        ...
        for i in xrange(len(t_rv)):
            ...
            normalized_CCF = np.array(CCF_sum/max(CCF_sum))
            ...
            if RVdatadict.has_key('RV')\
                    ...
                    or RVdatadict.has_key('BiGauss')\
                    or RVdatadict.has_key('Vasy') :
                p0 = [contrast[np.argmax(flux*contrast/100.)], rv0[np.argmax(flux*contrast/100.),i], FWHM[np.argmax(flux*contrast/100.)]/2.3548]
                flag = 0
                try:
                    CCF_res, flag = fitgauss(RV_CCF, normalized_CCF, p0)
                except ValueError:
                    raise RVgaussianFitError("Problem with CCF fitting in PASTIS_RV.py")
