...
def get_FWHM(vsini, BmV, spectrograph):
	...
	if spectrograph == 'HARPS' : 
		...
		A=1.95
		FWHM=np.sqrt((vsini**2.+A**2.*sigma0_harps**2.)/A**2.)*(2.*np.sqrt(2.*np.log(2.)))
	if spectrograph == 'SOPHIE HE' : 
		sigma0_HE=10.52-22.56*BmV+22.37*BmV**2.-6.95*BmV**3.
		A=1.64
		FWHM=np.sqrt((vsini**2.+A**2.*sigma0_HE**2.)/A**2.)*(2.*np.sqrt(2.*np.log(2.)))
	if spectrograph == 'SOPHIE HR' : 
		sigma0_HR=9.90 - 22.56*BmV + 22.37*BmV**2. - 6.95*BmV**3.
		A=1.95
		FWHM=np.sqrt((vsini**2.+A**2.*sigma0_HR**2.)/A**2.)*(2.*np.sqrt(2.*np.log(2.)))
	return FWHM
...
def get_contrast(FWHM, BmV, z, spectrograph, mask):
	...
	if spectrograph == 'HARPS':
		...
		contrast = 10**logW / np.sqrt(2.*np.pi) / (FWHM / (2.*np.log(2.))) * 100. # in % 
...
def make_CCF(rv, rv0, FWHM, contrast, flux):
	...
	import numpy as np
	sigma = FWHM / 2.3548
	return (1.-contrast/100.*np.exp(-(rv-rv0)**2/(2.*sigma**2)))*flux
...
def PASTIS_CCF(t_rv, RV_CCF, spectrograph, *args):
	...
	normalized_CCF = np.zeros([r_rv, RV_CCF], float)
	if 1 :
  ...
  # Make fixed CCFs
		...
		for i in xrange(len(t_rv)):
			...
			normalized_CCF[i] = np.array((CCF_sum-min(CCF_sum))/max(CCF_sum-min(CCF_sum)))
