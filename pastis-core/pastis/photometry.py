...
def initialize_phot(
    pbands=Allpbands, ZEROMAGfile=zeromagfile, FILTERdir=filterpath, AMmodel="BT"
):
...
def read_CK_spectra():
    ...
    for i, zz in enumerate(AMz):
        ...
        for j, teff in enumerate(AMteff):
            ...
            fitsfilename = os.path.join(
                libpath, "AM", "ck04models", parentdirectory, filename
            )
...
def read_BTsettl_spectra():
    ...
    AMspectra[(0.0, 700, 5.5)] = 0.5 * (
        AMspectra[(0.0, 600, 5.5)] + AMspectra[(0.0, 800, 5.5)]
    )
    AMspectra[(0.0, 1000, 3.5)] = 0.5 * (
        AMspectra[(0.0, 900, 3.5)] + AMspectra[(0.0, 1100, 3.5)]
    )
...
def read_WD_spectra():
    ...
    for kk in WDspectra.keys():
        ...
        ff = interpolate.interp1d(
            WDww, WDspectra[kk], bounds_error=False, fill_value=0.0
        )(ww)
...
def read_wwHR():
    ...
    if not "wwHR" in globals():
        f = open(
            os.path.join(libpath, "AM", "BT-Settl", "Flux_envolvente", "lambda.pickle")
        )
...
def get_interpolated_AM(z, teff, logg, HR=False):
    ...
    if indz == 0 or indz == infoAM["lenAMz"]:
        raise SpectrumInterpolError(
            "Metallicity (z = %.2f) outside grid." % z,
            indz,
            indteff,
            indlogg,
            outside=True,
    ...
    if indteff == 0 or indteff == infoAM["lenAMteff"]:
        raise SpectrumInterpolError(
            "Effective temperature (teff = %d) outside grid." % teff,
            indz,
            indteff,
            indlogg,
            outside=True,
    ...
    if indlogg == 0 or indlogg == infoAM["lenAMlogg"]:
        raise SpectrumInterpolError(
            "Log(g) (%.2f) outside grid." % logg, indz, indteff, indlogg, outside=True
        )
        ...
        for j, iteff in zip(range(2), (indteff - 1, indteff)):
            for k, ilogg in zip(range(2), (indlogg - 1, indlogg)):
                if HR:
                    try:
                        spectra[i, j, k] = (
                            read_HR_AM(AMz[iz], AMteff[iteff], AMlogg[ilogg])
                            / AMteff[iteff] ** 4
                        )
                    except TypeError:
                        raise SpectrumInterpolError(
                            "Missing node in grid.", indz, indteff, indlogg
                        )

                else:
                    try:
                        spectra[i, j, k] = AMspectra[
                            (AMz[iz], AMteff[iteff], AMlogg[ilogg])
                        ]
                    except KeyError:
                        raise SpectrumInterpolError(
                            "Missing node in grid.", indz, indteff, indlogg
...
def get_nearest_AM(z, teff, logg, HR=False, outsidegrid=None, isindex=False):
    """
    ...
    if outsidegrid:
        ...
        if z < min(AMz):
            print(
                "get_nearest_AM: Metallicity (z = %.2f) outside grid, using minimum value (%.2f)."
                % (z, min(AMz))
            )
            indz = 0
            ...
            print(
                "get_nearest_AM: Metallicity (z = %.2f) outside grid, using maximum value (%.2f)."
                % (z, max(AMz))
            )
            indz = max(iz)
        ...
        if teff < min(AMteff):
            print(
                "get_nearest_AM: Effective temperature (teff = %d) outside grid, using minimum value (%d)."
                % (teff, min(AMteff))
            )
            indteff = 0
            ...
            print(
                "get_nearest_AM: Effective temperature (teff = %d) outside grid, using maximum value (%d)."
                % (teff, max(AMteff))
            )
            indteff = max(iteff)
        ...
        if logg < min(AMlogg):
            print(
                "get_nearest_AM: Surface gravity (logg = %.2f) outside grid, using minimum value (%.2f)."
                % (logg, min(AMlogg))
            )
            indlogg = 0
            ...
            print(
                "get_nearest_AM: Surface gravity (logg = %.2f) outside grid, using maximum value (%.2f)."
                % (logg, max(AMlogg))
            )
            indlogg = max(ilogg)
...
def interpol_filter(ww, FILTERdir, photband="CoRoT"):
    ...
    if photband == "Kepler":
        x, y = loadtxt_iter(
            os.path.join(FILTERdir, photband + ".dat"), unpack=True, skiprows=1
        )
        x = x * 10.0
    ...
    # Interpolate filter
    trans = interpolate.interp1d(x, y, bounds_error=False, fill_value=0.0)(ww)
...
def get_interpolated_WD(teff, logg):
    ...
    indteff0 = int(indteff)
    indteff1 = int(indteff + 1.0)
    indlogg0 = int(indlogg)
...
def get_nearest_WD(teff, logg):
    ...
    if indcheck:
        ...
        if teff < min(WDteff):
            print(
                "get_nearest_WD: Effective temperature (teff = %d) outside grid, using minimum value (%d)."
                % (teff, min(WDteff))
            )
            indteff = 0
            ...
            print(
                "get_nearest_WD: Effective temperature (teff = %d) outside grid, using maximum value (%d)."
                % (teff, max(WDteff))
            )
            indteff = max(iteff)
        ...
        if logg < min(WDlogg):
            print(
                "get_nearest_WD: Surface gravity (logg = %.2f) outside grid, using minimum value (%.2f)."
                % (logg, min(WDlogg))
            )
            indlogg = 0
            ...
            print(
                "get_nearest_WD: Surface gravity (logg = %.2f) outside grid, using maximum value (%.2f)."
                % (logg, max(WDlogg))
            )
            indlogg = max(ilogg)
