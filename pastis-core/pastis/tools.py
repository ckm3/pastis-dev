...
def rebin(x, N):
    ...
    Nx = len(x)
    nxb = int(np.ceil(float(Nx) / N))
    xb = np.zeros(nxb)
    for i, j in zip(range(0, Nx, N), np.arange(nxb)):
        xb[j] = (x[i : i + N]).mean()
...
def rebin_texp(over_t, over_y, init_t, texp):
    ...
    if len(over_t) != len(over_y):
        raise IOError(
            "arguments over_t (%i) and over_y (%i) should have the same dimension"
            % (len(over_t), len(over_y))
        )
    if np.isscalar(texp):
        texp = np.zeros(len(init_t), float) + texp
    elif len(texp) != len(init_t):
        raise IOError(
            "arguments texp (%i) and init_t (%i) should have the same dimension"
            % (len(texp), len(init_t))
        )

    init_y = np.zeros(len(init_t))
    for i in xrange(len(init_t)):
        init_y[i] = np.mean(
            over_y[
                np.where(
                    np.logical_and(
                        over_t >= init_t[i] - texp[i] / 2.0 / 86400.0,
                        over_t <= init_t[i] + texp[i] / 2.0 / 86400.0,
                    )
                )
            ]
...
def trilinear_interpolation(input_array, indices):
    ...
    if x1 >= input_array.shape[0]:
        x1 = input_array.shape[0] - 1
    if y1 >= input_array.shape[1]:
        y1 = input_array.shape[1] - 1
    if z1 >= input_array.shape[2]:
        z1 = input_array.shape[2] - 1
...
def eccanomaly(M, ecc, method="Newton", niterationmax=1e4, tol=1e-8):
    ...
    while np.any(np.abs(E - Eo) > tol / len(E)) or niteration == 0:
...
def trueanomaly(ea, ecc):
    ...
    """
    nu = 2.0 * np.arctan2(
        np.sqrt(1.0 + ecc) * np.sin(ea / 2.0), np.sqrt(1.0 - ecc) * np.cos(ea / 2.0)
    )
...
def sigma_clipping(y, k, ey=None, niter=1, useMAD=False):
    ...
    if ey != None:
        ...
        w = np.zeros(len(y)) + 1.0

    # Array that will contain condition
    cond = np.zeros(len(y), bool)

    # Masked arrays
    y = np.ma.array(y, mask=cond)
    w = np.ma.array(w, mask=cond)
    ...
    for i in range(niter):
        ...
        if useMAD:
            sigma = np.median(np.abs(y - mean_y)) * 1.4826

        else:
            sigma = np.sqrt(np.average((y - mean_y) ** 2.0, weights=w))

        ## Condition to mask arrays (i.e. those elements above or below k*sigma
        condi = np.greater(np.abs(y - mean_y), k * sigma)
...
def density_prior(
    teff, sigma_teff, logg, sigma_logg, z, sigma_z, N=1e3, verbose=False, EM="Dartmouth"
):
    """
    ...
    for i in xrange(int(N)):
        ...
        try:
            s1 = ac.Target(z=z[i], teff=Teff[i], logg=logg[i], dist=1.0, ebmv=0.0)
...
def fithist(y, p0, nbins=50, disttype="Normal", plot=True, **kwargs):
    ...
    ## Construct histogram
    N, binedges = np.histogram(y, bins=nbins, normed=True)
    x = binedges[:-1] + 0.5 * np.diff(binedges)

    x = x[N != 0]
    N = N[N != 0]

    # Compute error in bin numbers
    eN = np.sqrt(N) / sqrt(len(y) * (x[1] - x[0]))

    def residuals(p, x, y, ey):
        return (y - distdict[disttype][0].pdf(x, *p)) / ey

    popt, epopt, infodict, mesg, wf = optimize.leastsq(
        residuals, p0, args=(x, N, eN), full_output=True, **kwargs
    )
...
def leastsq(pastisfile, vd0={}, initialize=True, plot=False, **kwargs):
    """
    ...
    def resLM(p, keys, input_dict, datadict):
        ...
        try:
            res = PASTIS_MCMC.get_likelihood(
                X, input_dict, datadict, labeldict, False, False, LM=True
            )
    ...
    popt, cov, infodict, mesg, ier = optimize.leastsq(
        resLM, p0, args=(k, input_dict, datadict), full_output=True, **kwargs
    )
    ...
    if ier in [1, 2, 3, 4]:
        ...
        print("====")
        print(
            "Chi2 = %.4f for %d points."
            % (np.sum(infodict["fvec"] ** 2), len(infodict["fvec"]))
        )
...
def get_E0(ecc, omega):
    ...
    return 2.0 * np.arctan2(
        np.sin(nu0 / 2.0) * np.sqrt(1 - ecc), np.cos(nu0 / 2.0) * np.sqrt(1 + ecc)
    )
...
def get_Ea(ecc, omega):
    ...
    """
    nu_a = -(pi / 2.0 + omega)
    return 2.0 * np.arctan2(np.tan(nu_a / 2.0) * np.sqrt(1 - ecc), np.sqrt(1 + ecc))
...
def get_T0(ecc, omega, P, Tp):
    ...
    T0 = Tp + P / (2.0 * np.pi) * (E_0 - ecc * np.sin(E_0))
    return T0
...
def get_Tp(ecc, omega, P, T0):
    ...
    """
    E_0 = get_E0(ecc, omega)
    Tp = T0 - P / (2.0 * np.pi) * (E_0 - ecc * np.sin(E_0))
    return Tp
...
def get_T02(ecc, omega, P, Tp):
    ...
    Ta = Tp + P / (2.0 * np.pi) * (E_a - ecc * np.sin(E_a))
...
def iterative_mass(K, P, Ms, ecc, y, omega=None, Rs=None, use_b=False, **kwargs):
    """
    ...
    if not use_b:
        ...
        x0 = np.mean(
            K_ms
            * (P_s * Ms**2 / (2 * pi * G)) ** (1.0 / 3.0)
            * np.sqrt(1 - ecc**2)
            / np.sin(incl)
        )

        mact = scipy.optimize.newton(
            ff, x0, fprime=dff, args=(K_ms, P_s, Ms, ecc, incl), **kwargs
        )
        ...
        if omega is not None and Rs is not None:
            ...
            raise TypeError(
                "omega and Rs are needed if impact parameter is" " given as input."
            )
        ...
        def auxf(m2, K_ms, P_s, Ms, ecc, xx):
            return (
                ...
                * P_s ** (1.0 / 3.0)
                * sqrt(1 - ecc**2)
                / ((2 * pi * G) ** (1.0 / 3.0) * sqrt((Ms + m2) ** (2.0 / 3.0) - xx**2))
            )
        ...
        def dff(m2, K_ms, P_s, Ms, ecc, xx):
            return (
                auxf(m2, K_ms, P_s, Ms, ecc, xx)
                * (
                    1
                    - (Ms + m2) ** (2.0 / 3.0)
                    / (3.0 * ((Ms + m2) ** (2.0 / 3.0) - xx**2))
                )
                - 1.0
            )
        ...
        x0 = (
            ...
            * sqrt(1 - ecc**2)
            * Ms
            / ((2 * pi * G) ** (1.0 / 3.0) * sqrt(Ms ** (2.0 / 3.0) - xx**2))
        )

        mact = scipy.optimize.newton(
            ff, x0, fprime=dff, args=(K_ms, P_s, Ms, ecc, xx), **kwargs
        )
...
def fsincos(x, *p):
    ...
    """
    A, B, shift = p
    return A * np.sin(2.0 * np.pi * x) + B * np.cos(2.0 * np.pi * x) + shift
...
def fitoc(
    ...
    # Period array
    if pmax == None:
        pmax = 0.5 * (np.max(time) - np.min(time))
    if logscale:
        periods = np.logspace(np.log10(pmin), np.log10(pmax), nperiods)
    else:
        periods = np.linspace(pmin, pmax, nperiods)

    nchain = len(vd["FitPlanet1_P"])
    nperiods = len(periods)
    data = np.zeros((nchain, nperiods))
    ...
    for i in range(nchain):
        ...
        for j in range(nperiods):
            ...
            coeff, var_matrix = curve_fit(
                fsincos, phase, oc, p0=[0.1, 0.1, 0.1], sigma=srv
            )
            # error de K = n.sqrt(var_matrix[0][0])
            K = np.sqrt(coeff[0] ** 2 + coeff[1] ** 2)
            data[i, j] = (
                K * 1e3 * (vdmass[i] ** (2.0 / 3.0)) * (periods[j] ** (1.0 / 3.0))
            ) / 203.0  # Mjup
...
def fitoc(
    ...
    if logscale:
        ...
        periods = np.linspace(pmin, pamx, nperiods)
...
def plotfitoc(pdata="simres.pickle", log=False):
    ...
    Mass = np.zeros((nperiods, 3))

    for i in range(nperiods):
        perioddata = data[:, i]
        Mass[i, 0] = np.percentile(perioddata, 68.27)
        Mass[i, 1] = np.percentile(perioddata, 95.45)
        Mass[i, 2] = np.percentile(perioddata, 99.73)
    ...
    ax.set_ylabel(
        "Upper limit 2nd planet mass [M$_J$]", fontsize=fontsize, labelpad=lpad
    )
...
def fitoc2(
    ...
    nchain = len(vd["FitPlanet1_P"])
    nperiods = len(periods)
    data = np.zeros((nsamples, nperiods))
...
def leastsqlin(x, y, ey, f, *args, **kwargs):
    ...
    return np.array(par)[:, 0], np.sqrt(np.diag(vpar)), vpar
...
def loadtxt_iter(
    ...
    data = np.fromiter(iter_func(usecols=usecols), dtype=dtype)
