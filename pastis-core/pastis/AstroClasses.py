...
try:
    ...
    print(
        "Could not import stellar atmosphere models. Are you only using "
        "FitObs objects?"
    )
...
class Star(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.alphaS = kwargs.pop("alphaS", 0.0)  # Effective filling factor due to spots

        if spotmodel == "Macula":
            # Parameters for spot modeling
            for spotstarparams in [
                "rotangle",
                "period",
                "kappa2",
                "kappa4",
                ...
                self.__setattr__(spotstarparams, kwargs.pop(spotstarparams, None))
                ...
                for i in range(self.Nspots):
                    for spotparam in [
                        ...
                        self.__setattr__(
                            ...
                            kwargs.pop("spot" + str(i) + spotparam, None),
                        )
    ...
    def test_critical(self, attrs, names):
        ...
        if len(missing) != 0:
            s = ["{} ({}) not provided".format(nam, cri) for cri, nam in missing]
        ...
        if spotmodel == "Macula" and self.Nspots > 0:
            ...
            for i in range(self.Nspots):
                for j, spotparam in enumerate(
                    ...
                    spot[j][i] = self.__getattribute__("spot" + str(i) + spotparam)
                    if (
                        spotparam == "lambda0"
                        or spotparam == "phi0"
...
class PlanetHost(Star):
    def __init__(self, **kwargs):
        ...
        self.test_critical(
            ["teff", "dens"], ["Effective temperature", "Mean stellar density"]
        )
    ...
    def get_stellarparameters(self, output=False):
        ...
        mact, logL, logage = iso.get_stellarparams_target(
            self.z, self.dens, log10(self.teff), planethost=True
        )
        self.mact = mact
        self.logage = logage
        self.L = 10**logL
...
class Planet(object):
    ...
    def __init__(self, orbital_parameters, Mp, Rp, f=0.0, **kwargs):
...
class PlanSys(object):
    def __init__(self, star, *planets, **kwargs):
        ...
        for planet in planets:
            if not (isinstance(planet, Planet) or isinstance(planet, FitPlanet)):
                print(
    ...
    def get_K(self, planet):
        ...
        if isinstance(planet, Planet):
            ...
            print(
                "Something went wrong. One of the orbiting objects in this "
    ...
    def get_RV(self, t, isphase=False):
        ...
        rv = (
            ...
            + (
                ...
                + self.star.drift2 * (t - TrefRV) ** 2
                + self.star.drift3 * (t - TrefRV) ** 3
            )
        )
    ...
    def get_LC(self, t, photband="Kepler", isphase=False, dt0=0.0):
        ...
        if len(self.planets) > 1 and isphase:
            raise Exception("Can't work on phase if more than one planet is present.")
        ...
        for planet in self.planets:
            ...
            if beaming:
                ...
                y = y[0] * (1.0 - B1 * VR1 / c) + y[1] - y[2] + y[3]
...
class FitBinary(object):
            ...
            if not isinstance(self, IsoBinary):
                ...
                for comp in ["comp1", "comp2"]:
                    for spotstarparams in [
                        ...
                        self.__setattr__(
                            comp + spotstarparams,
                            kwargs.pop(comp + spotstarparams, None),
                        )

                    if (
                        self.__getattribute__(comp + "c1") is None
                        or self.__getattribute__(comp + "c2") is None
                        or self.__getattribute__(comp + "c3") is None
                        or self.__getattribute__(comp + "c4") is None
                    ):
                        if (
                            self.__getattribute__("ua" + comp[-1]) is not None
                            and self.__getattribute__("ub" + comp[-1]) is not None
                        ):
                            # Impose quadratic limb-darkening from star
                            self.__setattr__(comp + "c1", 0)
                            self.__setattr__(
                                comp + "c2",
                                self.__getattribute__("ua" + comp[-1])
                                + 2 * self.__getattribute__("ub" + comp[-1]),
                            )
                            self.__setattr__(comp + "c3", 0)
                            self.__setattr__(
                                comp + "c4", -self.__getattribute__("ub" + comp[-1])
                            )
                        ...
                        for i in range(self.__getattribute__(comp + "Nspots")):
                            for spotparam in [
                                ...
                                self.__setattr__(
                                    ...
                                    kwargs.pop(
                                        comp + "spot" + str(i) + spotparam, None
                                    ),
                                )
    ...
    def get_LD(self, photband, component="primary"):
        ...
        if (isinstance(ua, float) or isinstance(ua, int)) and (
            ...
            try:
                ...
                raise KeyError(
                    "No limbdarkening provided for photband " "%s" % photband
                )
    ...
    def get_GD(self, photband, component="primary"):
        ...
        if isinstance(gd, float) or isinstance(gd, int):
            ...
            try:
                ...
                raise KeyError("No gravity darkening for photband %s" % photband)

        else:
            raise TypeError("Invalid gravity darkening for photband %s" % photband)
    ...
    def get_LC(self, t, photband="Kepler", isphase=False, dt0=0.0):
        ...
        if beaming:
            ...
            # neglecting beaming effect in reflection
            y = y1 * (1.0 - B1 * VR1 / c) + y2 * (1.0 - B2 * VR2 / c) - y[2] + y[3]
...
class IsoBinary(FitBinary):
    ...
    def __init__(self, orbital_parameters, Star1, Star2):
        ...
        ar = (
            G
            / (4.0 * pi**2)
            * (Star1.mact + Star2.mact)
            * Msun
            * (orbital_parameters.P * 24 * 3600.0) ** 2
        ) ** (1.0 / 3.0) / (Star1.R * Rsun)
    ...
    def get_sbr(self, photband):
        ...
        return np.double(
            ...
            * (
                1.0
                - ldc1[0] / 3.0
                - ldc1[1] / 6.0 / (1.0 - ldc2[0] / 3.0 - ldc2[1] / 6.0)
            )
        )
    ...
    def __init__(self, **kwargs):
        ...
        if self.secos is not None and self.sesin is not None:
            ...
            if self.ecc is not None or self.omega is not None:
                ...
                print(
                    "WARNING! Langrangean orbital elements and eccentricity "
        ...
        if self.Tp is not None:
            ...
            if self.T0 is not None:
                ...
                print(
                    "WARNING! Both Tp and T0 given. Will keep consistency " "with Tp."
                )
            try:
                self.T0 = self.get_T0()
            except AttributeError:
        ...
        elif self.epoch is not None and (self.M0 is not None or self.L0 is not None):

            if self.L0 is not None and self.Omega is not None:

                if self.M0 is not None:
                    print(
                        "WARNING! Both mean anomaly at epoch and mean "
