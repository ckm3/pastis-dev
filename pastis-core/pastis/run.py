...
def run_sim(pastisfile, pastisversion, submit=True, profiler=False, fullsave=False):
    ...
    betas = np.zeros(infodict["Nbeta"], float)
    chains = np.zeros(infodict["Nbeta"], float)
    if infodict["Nbeta"] == 1:
        betas += infodict["beta"]
        ...
        # betas[2]=1e-4
        betas[0] = 1e-4
        betas[1:] = np.logspace(-2.0, 0.0, infodict["Nbeta"] - 1.0)
        chains = np.linspace(1.0, infodict["Nchain"], infodict["Nbeta"])
    for beta, nc in zip(betas, chains):
        for c in range(int(round(nc))):

            # time.sleep(0.5)
            filename = infodict["name"] + "_" + (infodict["comment"]).replace(" ", "_")
            f = open(os.path.join(runpath, infodict["name"], filename + ".py"), "w")
            f.write("'''\n")
            ...
            f.write(
                "outfile = os.path.join(PASTISroot, 'resultfiles', "
                "'{}','{}'+sufix+'.mcmc')\n\n".format(
                    infodict["name"], filename + "_started"
                )
            )
            ...
            if infodict["Nsave"] is not None:
                ...
                f.write("outfilepart = outfile.replace('.mcmc', '.part.mcmc')\n\n")
                if profiler:
                    f.write("import cProfile\n\n")
                    f.write("outproffile = outfile.replace" "('.mcmc','.prof')\n\n")
                    f.write(
                        "cProfile.runctx('C = PASTIS_MCMC.mcmc(input_dict, datadict, dd[3], "
                        + str(infodict["Nmcmc"])
                        + ", beta="
                        + str(beta)
                        + ", Npca="
                        + str(infodict["Min_PCA"])
                        + ", NupdatePCA="
                        + str(infodict["N_update_PCA"])
                        + ", Nlastupdate="
                        + str(infodict["Max_PCA"])
                        + ", BIpca="
                        + str(infodict["BI_PCA"])
                        + ", randomstart="
                        + str(infodict["randomstart"])
                        + ", usePCA="
                        + str(infodict["PCA"])
                        + ", Nsave="
                        + str(infodict["Nsave"])
                        + ", outputfile=outfilepart)', globals(), locals(),  filename = outproffile)\n\n"
                    )
                else:
                    runparams = {
                        "steps": int(infodict["Nmcmc"]),
                        "beta": beta,
                    ...
                    f.write(
                        "C = PASTIS_MCMC.mcmc(input_dict, datadict, dd[3],"
                ...
                if profiler:
                    ...
                    f.write(
                        ...
                        + ", usePCA = "
                        + str(infodict["PCA"])
                        + ")', globals(), locals(),  filename = outproffile)\n\n"
                    )
                else:
                    runparams = {
                        "steps": int(infodict["Nmcmc"]),
            ...
            if infodict["Nsave"] != None and infodict["Nmcmc"] >= infodict["Nsave"]:
            ...
            if submit:
                qsubname = (
                    (infodict["name"] + " " + infodict["comment"]).replace(" ", "_")
                    + "_rep" % beta
                    + str(c).zfill(4)
                )
                ...
                qsubsubmit = (
                    "sbatch -p {queue} -J {jobname} -t {walltime} "
