...
def update_list_jumpparams(self):
	...
	self.list_jumpparams = np.sort(self.list_jumpparams)

def update_mcmc_listbox(self):
	import numpy as np
	self.resultbox.delete(0, END)
	for f in np.sort(self.chaindict.keys()) :
...
def result_remove_chains2(self):
	...
	if question :
		r = np.sort(self.chaindict.keys())[self.highlightchain.get()]
		del self.chaindict[r]
	self.update_mcmc_listbox()
	self.wplot.destroy()
...
def result_remove_chains3(self):
	...
	if question :
		for i,r in enumerate(np.sort(self.chaindict.keys())):
...
def get_plot(self, BI):
	...
	if self.resultvariable.get() <> 'ALL' : 
		...
		for k in np.sort(self.chaindict.keys()):
...
def remove_not_converged_chains(self):
	...
	medLogL, stdLogL = np.zeros(n_chains, float), np.zeros(n_chains, float)
	self.startindex_converged = round(float(self.BIconverged.get())/100.*len(self.chaindict[self.chaindict.keys()[0]]['logL']))
	for i,k in enumerate(np.sort(self.chaindict.keys())):
		medLogL[i] = np.median(self.chaindict[k]['logL'][self.startindex_converged:])
		stdLogL[i] = np.std(self.chaindict[k]['logL'][self.startindex_converged:])
	maxLogL = max(medLogL)
	self.chains_to_remove = np.where(medLogL < maxLogL - self.result_sigma.get() * np.sqrt(stdLogL[np.argmax(medLogL)]**2. + stdLogL**2))[0]
...
def remove_late_converged_chains(self):
	...
	BI = []
	vd = []
	for f in np.sort(self.chaindict.keys()): vd.append(self.chaindict[f])
	...
	BI = np.array(BI)
	n_chains = len(vd)
	maxBI = np.zeros(n_chains, float)
	for i in xrange(n_chains):
		maxBI[i] = max(BI[:,i])
	self.chains_to_remove = np.where(maxBI > self.BIlate.get()/100.)[0]
	if len(self.chains_to_remove) > 0 : self.result_remove_chains3()
	else : print 'no chain to remove'
	self.maxBI = maxBI[np.where(maxBI <= self.BIlate.get()/100.)[0]]
