    ...
    for i, zz in enumerate(BTz):
        ...
        for j, teff in enumerate(BTteff):
            ...
            for k, logg in enumerate(BTlogg):
                ...
                if teff < 26:
                    filename = "lte%03d%+.1f%+.1f.BT-Settl.dat.bz2" % (teff, logg, zz)

                else:
                    filename = "lte%03d%+.1f%+.1fa+%s.BT-Settl.dat.bz2" % (
                        teff,
                        logg,
                        zz,
                        alpha,
                ...
                ffHR = interpolate.interp1d(
                    wave, flux, bounds_error=False, fill_value=0.0
                )(wwHR)
        ...
        for j, teff in enumerate(BTteff):
            ...
            for k, logg in enumerate(BTlogg):
                ...
                fileout = os.path.join(
                    outdir, "BT" + str(zzk) + "_" + str(teffk) + "_" + str(loggk)
                )
...
def WD_spectra():
    ...
    for j, teff in enumerate(WDteff):
        ...
        for k, logg in enumerate(WDlogg):
            ...
            filename = str(
                "da"
                + str(long(WDteff[j])).zfill(5)
                + "_"
                + str(long((WDlogg[k] * 100.0))).zfill(3)
                + ".dk"
            )
