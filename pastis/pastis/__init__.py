...
ClassesList = {
    "Star": {
        ...
        "chromatic": ["albedo", "ua", "ub", "gd", "B", "alphaS"],
        "iterable": {},
    },
    "PlanetHost": {
        "achromatic": ["teff", "dens", "z", "dist", "vsini", "v0", "ebmv"],
        "chromatic": ["albedo", "ua", "ub", "gd", "B", "alphaS"],
        "iterable": {},
    },
    "Blend": {
        "achromatic": ["minit", "z", "logage", "dist", "vsini", "v0", "ebmv"],
        "chromatic": ["albedo", "ua", "ub", "gd", "B", "alphaS"],
        "iterable": {},
    },
    "Target": {
        "achromatic": ["teff", "logg", "z", "dist", "vsini", "v0", "ebmv"],
        "chromatic": ["albedo", "ua", "ub", "gd", "B", "alphaS"],
        "iterable": {},
    },
    "WhiteDwarf": {
    ...
    "Planet": {
        "achromatic": ["Mp", "Rp", "orbital_parameters"],
        "chromatic": ["albedo", "f"],
        "iterable": {},
    },
    "FitBinary": {
    ...
    "FitPlanet": {
        ...
        "chromatic": ["ua1", "ub1", "gd1", "B1", "albedo2"],
        "iterable": {},
    },
    "IsoBinary": {
        "achromatic": [
    ...
    "qBinary": {
        "achromatic": ["orbital_parameters", "q"],
        "chromatic": [
            "sbr",
            "ua1",
            "ub1",
...
TypeList = {
    "binary": ["FitBinary", "IsoBinary", "qBinary", "FitPlanet"],
    "plansys": [
        "PlanSys",
    ],
...
def initialize(*args):
    ...
    if len(args) == 0:
        ...
        photometry.initialize_phot(
            pbands, zeromagfile, filterpath, AMmodel=SAMdict["BT-settl"]
        )
        ...
        ld.initialize_limbdarkening(
            pbands_limbdarkening, ATMmodel="A", LDCfile=LDdict["Claret2011_wTESS"]
        )
    ...
    for kk in datadict.keys():
        if "filter" in datadict[kk]:
            if (
                datadict[kk]["filter"] == "CoRoT-R"
                or datadict[kk]["filter"] == "CoRoT-G"
                or datadict[kk]["filter"] == "CoRoT-B"
            ):
                corotcolor = True
            else:
    ...
    if "SED" in datadict or contains_target or contains_blend:
        photometry.initialize_phot(
            pbands, zeromagfile, filterpath, AMmodel=SAMdict[infodict["SAM"]]
        )
    ...
    ra = infodict["alpha"]
    dec = infodict["delta"]
    ext.initialize_extinction(ra, dec, infodict["MaxDist"], infodict["EXTstep"], Rv=3.1)
    ...
    if len(pbands_limbdarkening) > 0:
        ...
        ld.initialize_limbdarkening(
            pbands_limbdarkening, ATMmodel=atmmodel, LDCfile=LDdict[infodict["LDC"]]
        )
